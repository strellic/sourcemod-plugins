...
// ====[ DEFINES ]=============================================================
#define PLUGIN_VERSION "2.2.0_teal"

// ====[ HANDLES | CVARS ]=====================================================
new Handle:cvarEnabled;
new Handle:cvarImmunity;
new Handle:cvarSBRank;
...
public OnPluginStart()
{
	...
	cvarSBRank = CreateConVar("sm_muteonvote_sbrank", "5", "The rank at which players won't be muted (Ranks start at 0, -1 disables)");
...
stock bool:IsAdmin(iClient)
{
	...
	int rank = GetConVarInt(cvarSBRank);
	if(rank != -1)
	{
		if(SB_GetRank(iClient) >= rank)
		{
			return true;
		}
	}

	if(Leader_CurrentLeader() == iClient)
	{
		return true;
	}
