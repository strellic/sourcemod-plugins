...
int number, onumber;
Handle timerHandle = INVALID_HANDLE, HudSync, CountdownHUD_Cookie = INVALID_HANDLE;
...
public void OnPluginStart()
{
	...
	CountdownHUD_Cookie = RegClientCookie("cdhudoff_cookie", "Is CDHud off?", CookieAccess_Protected);
	...
	RegConsoleCmd("sm_cdhud", Command_CDHud, "Toggle CDHud");
}

public void OnClientCookiesCached(int client)
{
	char sValue[8];
	GetClientCookie(client, CountdownHUD_Cookie, sValue, sizeof(sValue));
	g_bStatus[client] = (sValue[0] == '\0' || !StringToInt(sValue));
}

public Action Command_CDHud(int client, int argc)
{
	char sValue[8];
	g_bStatus[client] = !g_bStatus[client];
	PrintToChat(client, "[SM] CDHud has been %s.", g_bStatus[client] ? "enabled" : "disabled");
	Format(sValue, sizeof(sValue), "%i", !g_bStatus[client]);
	SetClientCookie(client, CountdownHUD_Cookie, sValue);
